...
A store that remains colocated with the monitored target or its same mutable host/volume boundary is insufficient as the final audit trail; it may be local preview/spool state, but not the durable evidence boundary.
The fixed integrity/threat boundary for that distinction is documented in [`remote-audit-integrity-boundary.md`](remote-audit-integrity-boundary.md).
...
The checked-in ownership / next-step status explanation layer for that same control-plane slice is documented in [`approval-control-plane-status-explanation.md`](approval-control-plane-status-explanation.md).
The checked-in audit/export projection for that same control-plane slice is documented in [`approval-control-plane-audit-export.md`](approval-control-plane-audit-export.md).
The repository-level minimum packaging / deployment / observability / rollback posture for the current gap-closing phase is documented in [`deployment-hardening-minimums.md`](deployment-hardening-minimums.md).
The narrower separate-machine audit preview target inside that same phase, which separates runtime reliability, audit usability, coverage visibility, and deployment guidance for another Linux host evaluation, is documented in [`preview-readiness-boundary.md`](preview-readiness-boundary.md), and its current must-have / should-have / later prioritization is fixed in [`preview-readiness-gap-matrix.md`](preview-readiness-gap-matrix.md).
...
The next live boundary beneath generic REST / GWS / GitHub / messaging policy surfaces, which separates proxy seam, session correlation, semantic conversion, policy, approval, and audit responsibilities for intercepted requests, is documented in [`live-proxy-interception-foundation.md`](live-proxy-interception-foundation.md). The concrete redaction-safe request model at that proxy seam is documented in [`live-proxy-http-request-contract.md`](live-proxy-http-request-contract.md), the shared `agenta-core` input handed from that seam into later provider taxonomy is documented in [`generic-live-action-envelope.md`](generic-live-action-envelope.md), the checked-in generic REST live preview adapter on top of that seam is documented in [`generic-rest-live-preview-path.md`](generic-rest-live-preview-path.md), the checked-in provider semantic live preview adapters are documented in [`provider-live-preview-adapter-boundaries.md`](provider-live-preview-adapter-boundaries.md), the checked-in live mode semantics are documented in [`live-preview-mode-semantics.md`](live-preview-mode-semantics.md), the checked-in coverage / fail-open / unsupported visibility layer is documented in [`live-preview-coverage-visibility.md`](live-preview-coverage-visibility.md), the checked-in preview policy / approval / audit reflection path is documented in [`live-preview-record-reflection.md`](live-preview-record-reflection.md), the checked-in fixture catalog and smoke test coverage are documented in [`live-proxy-test-fixtures.md`](live-proxy-test-fixtures.md), and the current per-slice live coverage / fail-open / approval-hold posture is fixed in [`live-proxy-coverage-matrix.md`](live-proxy-coverage-matrix.md). The integrity/threat boundary that forbids treating a colocated monitored-runtime store as sufficient final audit evidence is documented in [`remote-audit-integrity-boundary.md`](remote-audit-integrity-boundary.md). The minimum deployment topology that moves `remote hostd` and the durable evidence path outside the monitored target boundary is documented in [`remote-audit-deployment-topology.md`](remote-audit-deployment-topology.md). The follow-on boundary that replaces the checked-in same-host local-volume observed-runtime handoff with a remote session-lineage + request-append contract is documented in [`observed-runtime-remote-ingress-contract.md`](observed-runtime-remote-ingress-contract.md). The checked-in tamper-evident strategy that chooses per-stream hash chaining plus signed remote-store checkpoints, and defines the integrity metadata needed for durable audit rows, is documented in [`remote-audit-tamper-evident-strategy.md`](remote-audit-tamper-evident-strategy.md). The boundary between stand-in Compose smoke and handoff-ready OpenClaw / Hermes verification is documented in [`real-runtime-audit-readiness-boundary.md`](real-runtime-audit-readiness-boundary.md), and the follow-on real-traffic evidence boundary that distinguishes fixture preview, observed request, and validated observation is documented in [`real-traffic-observation-boundary.md`](real-traffic-observation-boundary.md).
