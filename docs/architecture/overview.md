...
The narrower separate-machine audit preview target inside that same phase, which separates runtime reliability, audit usability, coverage visibility, and deployment guidance for another Linux host evaluation, is documented in [`preview-readiness-boundary.md`](preview-readiness-boundary.md).
