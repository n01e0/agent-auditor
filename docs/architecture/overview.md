...
The next live boundary beneath generic REST / GWS / GitHub / messaging policy surfaces, which separates proxy seam, session correlation, semantic conversion, policy, approval, and audit responsibilities for intercepted requests, is documented in [`live-proxy-interception-foundation.md`](live-proxy-interception-foundation.md). The concrete redaction-safe request model at that proxy seam is documented in [`live-proxy-http-request-contract.md`](live-proxy-http-request-contract.md), and the shared `agenta-core` input handed from that seam into later provider taxonomy is documented in [`generic-live-action-envelope.md`](generic-live-action-envelope.md).
