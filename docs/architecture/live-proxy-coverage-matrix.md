...
- it proves **one validated observation** for GitHub `repos.update_visibility` through `capture -> correlate -> classify -> policy -> audit`, with durable `observation_provenance` / `validation_status` inspection output
